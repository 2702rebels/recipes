...
**About the name.** This section and [§4](/docs/guides/ctre/selection) say "FOC" to mean torque-current control, as most teams do. Strictly, FOC is a way of commutating the motor, and Phoenix Pro also uses it for voltage and duty-cycle requests when `EnableFOC = true` (see [§1.6](/docs/guides/ctre/motor-constants#16-motor-constants-reference)). What sets this section apart is the *output family*: Amps instead of Volts.
...
|:---|:---|:---|:---|
| kS | Yes | Yes (via `StaticFeedforwardSign` config — see [§1.2.1](/docs/guides/ctre/foundation#121-the-staticfeedforwardsign-config)) | Yes |
...
with $\sigma_{\text{kS}}$ set by `StaticFeedforwardSign` ([§1.2.1](/docs/guides/ctre/foundation#121-the-staticfeedforwardsign-config)). Use `UseClosedLoopSign` here so kS acts on small corrections. With the default, $\sigma_{\text{kS}} = 0$ and kS does nothing.
...
with $\sigma_{\text{kS}}$ set by `StaticFeedforwardSign` ([§1.2.1](/docs/guides/ctre/foundation#121-the-staticfeedforwardsign-config)). Use `UseClosedLoopSign` without a profile: kG supplies the holding current, and kS only needs to act on small corrections, in whichever direction the error points.
...
Example: 5 kg arm with CG at 0.4 m from pivot, gear ratio 100:1, Kraken X60 in FOC mode ($k_t = 0.0194$ N·m/A from [§1.6.2](/docs/guides/ctre/motor-constants#162-derived-electromechanical-constants)):
...
3. **The same force up and down.** Current-to-force is identical in both directions. In voltage mode, back-EMF eats into headroom on fast up-moves while gravity helps on down-moves, so the loop feels different each way. Torque mode removes that difference (again within the voltage headroom). The *friction* difference of [§2.4.6](/docs/guides/ctre/voltage#246-asymmetric-friction) is mechanical and stays.
...
The state machine follows a position while extending and applies a set force while holding. Voltage mode can't do this cleanly. At a stall, current is $V/R$, and $R$ changes by around 25% as the motor warms up ([§1.6.4](/docs/guides/ctre/motor-constants#164-caveats)), so a fixed voltage doesn't give a fixed force.
