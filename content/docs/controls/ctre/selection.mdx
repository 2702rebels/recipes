...
1. **Every gain must be re-derived.** kP, kI, kD, kS, kV, kA, and kG all change units. SysId still works in torque mode, but its kV is nearly meaningless there ([§3.0.4](/docs/controls/ctre/torque-current#304-sysid-and-torque-current-what-actually-transfers)). The reliable path is to compute kG and kA from physics (motor $k_t$, gear ratio, mass, geometry) and measure kS with a slow current ramp. Plan on a solid session per mechanism.
...
3. **Motion Magic Expo configs stay in Volts.** `MotionMagicExpo_kV` and `MotionMagicExpo_kA` do not change units with the request family, unlike slot kV/kA ([§1.4.1](/docs/controls/ctre/motion-magic-expo#141-motion-magic-expo-in-detail)).
4. **WPILib's feedforward classes don't apply.** `ArmFeedforward`, `ElevatorFeedforward`, and `SimpleMotorFeedforward` work in volts. Compute current feedforwards from physics yourself, using the formulas on the [torque-current page](/docs/controls/ctre/torque-current).
